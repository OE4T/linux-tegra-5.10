...
static inline uint16_t lMulU16Cert (uint16_t factor1, uint16_t factor2, uint16_t failValue)
{
...
static inline uint16_t lSubU16Cert (uint16_t minuend, uint16_t subtrahend, uint16_t failValue)
{
...
static uint16_t CRC_CalculateCRC16(
    ...
    bool Crc_IsFirstCall)
{
    ...
    static const uint16_t sLookupTableCrc16[UINT8MAX + 1U] = {
        ...
        0xe1ce,	0xf1ef,	0x1231,	0x210,	0x3273,	0x2252,	0x52b5,	0x4294,	0x72f7,	0x62d6,	0x9339,	0x8318,	0xb37b,
        0xa35a,	0xd3bd,	0xc39c,	0xf3ff,	0xe3de,	0x2462,	0x3443,	0x420,	0x1401,	0x64e6,	0x74c7,	0x44a4,	0x5485,
        0xa56a,	0xb54b,	0x8528,	0x9509,	0xe5ee,	0xf5cf,	0xc5ac,	0xd58d,	0x3653,	0x2672,	0x1611,	0x630,	0x76d7,
        0x66f6,	0x5695,	0x46b4,	0xb75b,	0xa77a,	0x9719,	0x8738,	0xf7df,	0xe7fe,	0xd79d,	0xc7bc,	0x48c4,	0x58e5,
        0x6886,	0x78a7,	0x840,	0x1861,	0x2802,	0x3823,	0xc9cc,	0xd9ed,	0xe98e,	0xf9af,	0x8948,	0x9969,	0xa90a,
        0xb92b,	0x5af5,	0x4ad4,	0x7ab7,	0x6a96,	0x1a71,	0xa50,	0x3a33,	0x2a12,	0xdbfd,	0xcbdc,	0xfbbf,	0xeb9e,
        0x9b79,	0x8b58,	0xbb3b,	0xab1a,	0x6ca6,	0x7c87,	0x4ce4,	0x5cc5,	0x2c22,	0x3c03,	0xc60,	0x1c41,	0xedae,
        0xfd8f,	0xcdec,	0xddcd,	0xad2a,	0xbd0b,	0x8d68,	0x9d49,	0x7e97,	0x6eb6,	0x5ed5,	0x4ef4,	0x3e13,	0x2e32,
        0x1e51,	0xe70,	0xff9f,	0xefbe,	0xdfdd,	0xcffc,	0xbf1b,	0xaf3a,	0x9f59,	0x8f78,	0x9188,	0x81a9,	0xb1ca,
        ...
        0x14a0,	0x481,	0x7466,	0x6447,	0x5424,	0x4405,	0xa7db,	0xb7fa,	0x8799,	0x97b8,	0xe75f,	0xf77e,	0xc71d,
        0xd73c,	0x26d3,	0x36f2,	0x691,	0x16b0,	0x6657,	0x7676,	0x4615,	0x5634,	0xd94c,	0xc96d,	0xf90e,	0xe92f,
        0x99c8,	0x89e9,	0xb98a,	0xa9ab,	0x5844,	0x4865,	0x7806,	0x6827,	0x18c0,	0x8e1,	0x3882,	0x28a3,	0xcb7d,
        0xdb5c,	0xeb3f,	0xfb1e,	0x8bf9,	0x9bd8,	0xabbb,	0xbb9a,	0x4a75,	0x5a54,	0x6a37,	0x7a16,	0xaf1,	0x1ad0,
        0x2ab3,	0x3a92,	0xfd2e,	0xed0f,	0xdd6c,	0xcd4d,	0xbdaa,	0xad8b,	0x9de8,	0x8dc9,	0x7c26,	0x6c07,	0x5c64,
        0x4c45,	0x3ca2,	0x2c83,	0x1ce0,	0xcc1,	0xef1f,	0xff3e,	0xcf5d,	0xdf7c,	0xaf9b,	0xbfba,	0x8fd9,	0x9ff8,
    ...
    {
        lCrc = ((Crc_StartValue16 ^ ARLIB_CRC16_XOR_VALUE)& (uint16_t)UINT16MAX);
    }

    for (lIndex = 0U; lIndex < Crc_Length; lIndex++)
    {
        lLookUpVal = sLookupTableCrc16[Crc_DataPtr[lIndex] ^ (lCrc >> 8U)];
    ...
    uint16_t dataId)
{
    ...
    /* all necessary checks are already perfromed in calling fucntion
    ...
    if (offset > 0U)
    {
        ...
        lCrc = CRC_CalculateCRC16(&DataPtr[0U], offset, ARLIB_CRC16_START_VALUE, true);

        /* Compute CRC over bytes that are after CRC (if any) */
        lCrc = CRC_CalculateCRC16(&DataPtr[lOffset2], lLengthBytes, lCrc, false);
    }
    else
    {
        /* Compute CRC over bytes that are after CRC (if any) */
        lCrc = CRC_CalculateCRC16(&DataPtr[lOffset2], lLengthBytes, ARLIB_CRC16_START_VALUE, true);
    }

    lDataIdNibble = (uint8_t)((dataId) & (uint16_t)0x00FFU);
    lCrc = CRC_CalculateCRC16(&lDataIdNibble, 1U, lCrc, false);

    lDataIdNibble = (uint8_t)((dataId >> (uint8_t)8U) & (uint16_t)0x00FFU);
    lCrc = CRC_CalculateCRC16(&lDataIdNibble, 1U, lCrc, false);
    return lCrc;
}

static inline void lDoChecksP05(
...
static inline E2E_returntype_t lNullPtrCheck(
    const E2E_P05ConfigType *ConfigPtr,
    E2E_P05CheckStateType *StatePtr,
    const uint8_t* DataPtr,
    uint16_t Length)
...
static inline E2E_returntype_t lVerifyCheckInput(
    ...
    uint16_t Length)
{
    ...
    *NewDataAvailable = false;
    ...
    /* The offset shall be a multiple of 8 and 0 ≤ Offset ≤ MaxDataLength-(3*8). */
            ...
            (ConfigPtr->Offset > (MAX_P05_DATA_LENGTH_IN_BITS - MIN_P05_DATA_LENGTH_IN_BITS)) ||
            (ConfigPtr->Offset > lSubU16Cert(lLengthInBits, MIN_P05_DATA_LENGTH_IN_BITS, UINT16MAX)))
    {
        lRet = E2E_E_INPUTERR_WRONG;
    }
...
static inline E2E_returntype_t lVerifyProtectInput(
    const E2E_P05ConfigType *ConfigPtr,
 ...
 */
E2E_returntype_t E2E_P05ProtectInit(E2E_P05ProtectStateType *StatePtr)
{
 ...
 */
E2E_returntype_t E2E_P05CheckInit(E2E_P05CheckStateType *StatePtr)
{
    E2E_returntype_t lRet;
    lRet = E2E_E_OK;
 ...
 */
E2E_returntype_t E2E_P05Protect(
    const E2E_P05ConfigType *ConfigPtr,
    E2E_P05ProtectStateType *StatePtr,
    uint8_t* DataPtr,
    uint16_t Length)
{
    ...
    if (E2E_E_OK == lRet)
    {
        ...
        /* Computing Crc value */
        lCrc = lCalculateCRC(DataPtr, Length, lOffset, ConfigPtr->DataID);

        /* Updating Crc value in E2E header */
        DataPtr[lOffset] = (uint8_t)(lCrc  & UINT8MAX);
        DataPtr[lOffset+1U] = (uint8_t)((lCrc >> 8U)& (uint16_t)UINT8MAX);
    ...
    uint16_t Length)
{
    ...
    if (E2E_E_OK == lRet)
    {
        ...
        if (E2E_E_OK == lRet)
        {
            if (true == lNewDataAvailable)
            {
                /* offset of E2E header in data packet */
                lOffset = (ConfigPtr->Offset)/8U;

                lReceivedCounter = (DataPtr[lOffset+2U] & UINT8MAX);

                lReceivedCRC |= (((uint16_t)DataPtr[lOffset ]) & 0x00FFU);
                lReceivedCRC |= (((uint16_t)DataPtr[lOffset+1U] <<  8U) & (uint16_t)0xFF00U);

                lComputedCRC = lCalculateCRC(DataPtr, Length, lOffset, ConfigPtr->DataID);
            }
