...
static int oak_net_skb_tx_protocol_type(struct sk_buff* skb);
 ...
 *  */
int oak_net_esu_ena_mtu(struct net_device* net_dev, int new_mtu)
{
            ...
            if (err == 0)
            {
                ...
                if (err == 0)
                {
                    ...
                    if (err == 0)
                    {
                        ...
                        err = oak_net_esu_ena_mtu(np->netdev, np->netdev->mtu);
...
/* Name      : verify_rx_checksum
 * Returns   : int
 * Parameters:  oak_rx_chan_t * rxc = rxc,  oak_rxs_t * rsr = rsr
 *  */
int oak_net_verify_rx_checksum(oak_rx_chan_t* rxc, oak_rxs_t* rsr)
{
    /* automatically added for object flow handling */
    int rc_1 = CHECKSUM_NONE; /* start of activity code */

    if (rsr->vlan == 1)
    {
        /* UserCode{FC30478C-46F2-4f53-A41A-D40920733130}:1V2RK54f2B */
        ++rxc->stat.rx_vlan;
        /* UserCode{FC30478C-46F2-4f53-A41A-D40920733130} */
    }
    else
    {
    }
    ...
    if (rsr->l3_ipv4 == 1 || rsr->l3_ipv6 == 1)
    {
        ...
        if (rsr->l4_prot == 1 || rsr->l4_prot == 2)
        {

            if (rsr->l4_chk_ok == 1)
            {
                /* UserCode{E5F4E1DC-475D-4449-9628-6BFC52CB34E1}:QnvYRMGbKE */
                rxc->stat.rx_l4p_ok++;
                /* UserCode{E5F4E1DC-475D-4449-9628-6BFC52CB34E1} */

                rc_1 = CHECKSUM_UNNECESSARY;
            }
            else
            {
            }
 ...
 *  */
void oak_net_tx_heuristic(oak_tx_chan_t* txc, int len)
{
    /* start of activity code */

    if (len <= 64)
    {
        /* UserCode{8811EC74-FF8E-4e4d-B31F-38B06867C1A9}:WweJrUrt0z */
        ++txc->stat.tx_64;
        /* UserCode{8811EC74-FF8E-4e4d-B31F-38B06867C1A9} */
    }
    else
    {
        ...
        if (len <= 128)
        {
            /* UserCode{4978887F-6776-478f-9447-5C086896C9BB}:1Lj5P7Vevd */
            ++txc->stat.tx_128;
            /* UserCode{4978887F-6776-478f-9447-5C086896C9BB} */
        }
        else
        {
            ...
            if (len <= 256)
            {
                /* UserCode{978D483F-55C0-4771-A822-E63B7D9396B3}:b2r8JrA1pm */
                ++txc->stat.tx_256;
                /* UserCode{978D483F-55C0-4771-A822-E63B7D9396B3} */
            }
            else
            {
                ...
                if (len <= 512)
                {
                    /* UserCode{F39E6F37-EBFE-417b-9664-BDD9A9FDE1B6}:Dpu5b9Oc9o */
                    ++txc->stat.tx_512;
                    /* UserCode{F39E6F37-EBFE-417b-9664-BDD9A9FDE1B6} */
                }
                else
                {
                    ...
                    if (len <= 1024)
                    {
                        /* UserCode{1BFAE685-EC02-4f81-A003-55DADA1DB4D2}:FTVhcefdCK */
                        ++txc->stat.tx_1024;
                        /* UserCode{1BFAE685-EC02-4f81-A003-55DADA1DB4D2} */
                    }
                    else
                    {
                        /* UserCode{15B9D084-3F96-475b-A202-B6CC64B0D2AF}:1Q4wVM0im9 */
                        ++txc->stat.tx_2048;
                        /* UserCode{15B9D084-3F96-475b-A202-B6CC64B0D2AF} */
                    }
                }
            }
        }
 ...
 *  */
void oak_net_rx_heuristic(oak_rx_chan_t* rxc, int len)
{
    /* start of activity code */

    if (len <= 64)
    {
        /* UserCode{2A04834F-474D-4ce9-9CCF-9F046EB6B7AA}:F2JNSfEKmo */
        ++rxc->stat.rx_64;
        /* UserCode{2A04834F-474D-4ce9-9CCF-9F046EB6B7AA} */
    }
    else
    {
        ...
        if (len <= 128)
        {
            /* UserCode{361E7522-6C24-4eb6-BE0E-3DC16F3FA88E}:1IH6IvFIvJ */
            ++rxc->stat.rx_128;
            /* UserCode{361E7522-6C24-4eb6-BE0E-3DC16F3FA88E} */
        }
        else
        {
            ...
            if (len <= 256)
            {
                /* UserCode{CFA8E10D-F18D-47fe-BD86-F58B7C828C60}:YRdmMow1Wv */
                ++rxc->stat.rx_256;
                /* UserCode{CFA8E10D-F18D-47fe-BD86-F58B7C828C60} */
            }
            else
            {
                ...
                if (len <= 512)
                {
                    /* UserCode{355F3CBF-16CD-4a7c-9F39-D77A92F55044}:mSOqh906cF */
                    ++rxc->stat.rx_512;
                    /* UserCode{355F3CBF-16CD-4a7c-9F39-D77A92F55044} */
                }
                else
                {
                    ...
                    if (len <= 1024)
                    {
                        /* UserCode{3E101C26-AEB1-4969-8BB6-719E5753D90E}:1WdWpSsKd2 */
                        ++rxc->stat.rx_1024;
                        /* UserCode{3E101C26-AEB1-4969-8BB6-719E5753D90E} */
                    }
                    else
                    {
                        /* UserCode{DD555D04-4D58-43a1-ABC8-8C3FFCE36206}:6NFyewFmvn */
                        ++rxc->stat.rx_2048;
                        /* UserCode{DD555D04-4D58-43a1-ABC8-8C3FFCE36206} */
                    }
                }
            }
        }
...
static int oak_net_tx_packet(oak_t* np, struct sk_buff* skb, uint16_t txq)
{
    ...
    uint32_t nfrags;
    dma_addr_t mapping;
    int prot; /* automatically added for object flow handling */
    int rc_28 = 0; /* start of activity code */
    ...
    else
    {
        ...
        if (num > 0)
        {
            ...
            if (skb->ip_summed == CHECKSUM_PARTIAL)
            {
                ...
                if (prot == 2)
                {
                    /* UserCode{286A2C09-B0CA-4b26-B495-EB0CE9CB3A06}:B8HEuyQPnM */
                    cs_g4 = 1;
                    cs_g3 = 1;
                    /* UserCode{286A2C09-B0CA-4b26-B495-EB0CE9CB3A06} */
                }
                else if (prot == 1)
                {
                    /* UserCode{1935A1C4-4276-46ca-909C-991CF36D3224}:1ACyfY0LA5 */
                    cs_g3 = 1;
                    /* UserCode{1935A1C4-4276-46ca-909C-991CF36D3224} */
                }
                else
                {
	    ...
	    np->netdev->stats.tx_bytes += skb->len;
            ...
            oak_net_tx_heuristic(txc, skb->len);
...
static int oak_net_skb_tx_protocol_type(struct sk_buff* skb)
{
    uint8_t ip_prot = 0;
    int rc = 0;
    __be16 prot; /* automatically added for object flow handling */
    int rc_1 = 0; /* start of activity code */

    if (skb->protocol == htons(ETH_P_8021Q))
    {
        /* UserCode{E5E0B216-7FEC-4640-B689-23B6B33F9D24}:EGzFfHBiOP */
        prot = vlan_eth_hdr(skb)->h_vlan_encapsulated_proto;
        /* UserCode{E5E0B216-7FEC-4640-B689-23B6B33F9D24} */
    }
    else
    {
        /* UserCode{1554B15E-A5DA-433f-B70F-637125346E65}:zqrNvgyJHE */
        prot = skb->protocol;
        /* UserCode{1554B15E-A5DA-433f-B70F-637125346E65} */
    }

    if (prot == htons(ETH_P_IP))
    {
        /* UserCode{841024BC-7EF5-44ce-B483-963C13690A74}:1XP4eVMmO0 */
        ip_prot = ip_hdr(skb)->protocol;
        rc = 1;
        /* UserCode{841024BC-7EF5-44ce-B483-963C13690A74} */
    }
    else
    {

        if (prot == htons(ETH_P_IPV6))
        {
            /* UserCode{925B60BA-66A8-4e9f-B610-2FBE6A9815E3}:1AYPWHqt9C */
            ip_prot = ipv6_hdr(skb)->nexthdr;
            rc = 1;
            /* UserCode{925B60BA-66A8-4e9f-B610-2FBE6A9815E3} */
        }
    }

    if (ip_prot == IPPROTO_TCP || ip_prot == IPPROTO_UDP)
    {
        /* UserCode{C556982B-E124-4ce0-9C47-F1E1FB302231}:1QmLJONT1e */
        rc = 2;
        /* UserCode{C556982B-E124-4ce0-9C47-F1E1FB302231} */
    }
    else
    {
    }
        ...
        while ((desc_num > 0) && (comp_frame == 0))
        {
            ...
            if (comp_frame == 1)
            {
                ...
                if (good_frame == 1)
                {
                    ...
                    if (rsr->es == 0)
                    {
                        rxc->skb->ip_summed = oak_net_verify_rx_checksum(rxc, rsr);
                    }
                    else
                    {
                ...
                if (good_frame == 1)
                {
                    ...
                    oak_net_rx_heuristic(rxc, rxc->skb->len);
