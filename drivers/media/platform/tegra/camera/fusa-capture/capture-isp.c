 ...
 * @brief ISP channel operations for T186/T194
 *
...
/*
 * Capture ISP supporting functions (internal)
 */

int isp_capture_setup_syncpt(
	struct tegra_isp_channel *chan,
	const char *name,
	bool enable,
...
void isp_capture_release_syncpt(
	struct tegra_isp_channel *chan,
	struct syncpoint_info *sp)
{
	if (sp->id)
...
void isp_capture_release_syncpts(
	struct tegra_isp_channel *chan)
{
	struct isp_capture *capture = chan->capture_data;
...
int isp_capture_populate_fence_info(
	struct tegra_isp_channel *chan,
	int fence_offset,
...
int isp_capture_setup_inputfences(
	struct tegra_isp_channel *chan,
	struct isp_capture_req *req,
	int request_offset)
{
...
int isp_capture_setup_prefences(
	struct tegra_isp_channel *chan,
	struct isp_capture_req *req,
	int request_offset)
{
...
void isp_capture_request_unpin(
	struct tegra_isp_channel *chan,
...
void isp_capture_program_request_unpin(
	struct tegra_isp_channel *chan,
	uint32_t buffer_index)
{
	struct isp_capture *capture = chan->capture_data;
...
inline void isp_capture_ivc_capture_cleanup(
	struct isp_capture *capture,
	uint32_t buffer_index)
{
...
inline void isp_capture_ivc_capture_signal(
	struct isp_capture *capture,
	uint32_t buffer_index)
{
	if (capture->is_progress_status_notifier_set) {
...
inline void isp_capture_ivc_program_cleanup(
	struct isp_capture *capture,
	uint32_t buffer_index)
{
	struct tegra_isp_channel *chan = capture->isp_channel;
...
inline void isp_capture_ivc_program_signal(
	struct isp_capture *capture,
	uint32_t buffer_index)
{
	if (capture->is_progress_status_notifier_set) {
...
void isp_capture_ivc_status_callback(
	const void *ivc_resp,
	const void *pcontext)
{
...
int isp_capture_ivc_send_control(struct tegra_isp_channel *chan,
...
void isp_capture_ivc_control_callback(
	const void *ivc_resp,
	const void *pcontext)
{
