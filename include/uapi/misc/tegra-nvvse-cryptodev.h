...
struct  tegra_nvvse_aes_enc_dec_ctl {
	...
	/** Tag used for AES-GCM Decryption.
	 * During Decryption, the client shall populate tag generated
	 * at the time of Encryption
	*/
	uint8_t         gcm_tag[TEGRA_NVVSE_AES_GCM_TAG_SIZE];
	 ...
	 *  aad is optional, so when uAadLength is 0 this pointer can be NULL.
	 */

	uint8_t *pAadBuffer;
	/** [in] Holds the length of tag for GMAC.
	 *  supported uTagLength is 16 bytes
	 */

	uint32_t uTagLength;
	/** [inout] GMAC tag buffer
	 * During Encryption, tag of size uTagLength is generated by nvvse.
	 * During Decryption, tag of size uTagLength should be populated by the
	 * application.
	 */
	uint8_t *pTagBuffer;
};
